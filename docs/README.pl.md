...
## Telefon jako dodatkowe wejście
...
niego aplikację Kamera w iPhonie, a Safari otworzy stronę. Telefon i komputer
muszą być w tej samej sieci Wi-Fi.

Co telefon robi: przyjmuje **numer wpisany z etykiety karty**, pokazuje wynik —
przyznany / odmowa / nieznana — i listę ostatnich odczytów. Te odczyty
przechodzą **te same** reguły dostępu co odczyty z czytnika USB, trafiają do
historii ze stanowiskiem z sufiksem `/telefon` i pokazują się w panelu odczytu
w aplikacji, więc obsługa przy komputerze widzi je na bieżąco.

**Czego telefon nie potrafi:** nie odczyta karty zbliżeniowej. Safari nie ma Web
NFC (to API istnieje tylko w Chrome na Androidzie), więc strona internetowa na
iPhonie w ogóle nie ma dostępu do układu NFC. Odczyt kart wbudowanym czytnikiem
iPhone'a wymagałby osobnej aplikacji natywnej, podpisanej uprawnieniem NFC od
Apple — a to wymaga płatnego konta w Apple Developer Program. Karty zbliżeniowe
odczytuje czytnik USB przy komputerze.

Dla kart z nadrukowanym kodem QR istnieje druga droga: jeśli kod zawiera adres
`http://<host>:<port>/q/<sekret>/<numer>`, zeskanowanie go **systemową aplikacją
Kamera** rejestruje odczyt od razu, bez otwierania strony.
